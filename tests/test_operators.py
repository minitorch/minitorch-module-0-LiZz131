...
def test_sigmoid(a: float, b: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    if lt(a, b) and is_close(a, b) == 0.0:
        if is_close(sigmoid(a), 1.0) and is_close(sigmoid(b), 1.0):
            pass
        elif is_close(sigmoid(a), 0.0) and is_close(sigmoid(b), 0.0):
            pass
        else:
            assert lt(sigmoid(a), sigmoid(b))
